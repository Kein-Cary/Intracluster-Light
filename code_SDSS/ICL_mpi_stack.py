...
def stack_A_plane(band_number, subz, subra, subdec):
...
def stack_B_plane(band_number, subz, subra, subdec):
...
def main():
	...
	N_sum = np.array([3331, 3332, 3318, 3331, 3328])
	...
	for tt in range(3):
		...
		for aa in range(len(N_tt)):
			...
			if rank == cpus - 1:
				N_sub1 += n
			stack_A_plane(tt, set_z[N_sub0 :N_sub1], set_ra[N_sub0 :N_sub1], set_dec[N_sub0 :N_sub1])
			stack_B_plane(tt, set_z[N_sub0 :N_sub1], set_ra[N_sub0 :N_sub1], set_dec[N_sub0 :N_sub1])
			commd.Barrier()
	...
	if rank == 0:
		for tt in range(3):
			...
			for pp in range(cpus):
				...
				plt.figure()
				ax = plt.subplot(111)
				ax.set_title('A mask %s band %d cpus %d imgs' % (band[tt], pp, sub_Num) )
				...
				plt.savefig(home + 'fig_15sigma/stack_img/A_mask/A_sub_stack_%d_cpus_%s_band.png' % (pp, band[tt]), dpi = 300)
				plt.close()
			...
			with h5py.File(home + 'fig_15sigma/stack_img/A_mask/stack_Amask_%d_in_%s_band.h5' % (tot_N, band[tt]), 'w') as f:
	...
	if rank == 0:
		for tt in range(3):
			...
			with h5py.File(home + 'fig_15sigma/stack_img/A_mask/stack_Amask_%d_in_%s_band.h5' % (N_sum[tt], band[tt]), 'r') as f:
			...
			plt.figure()
			plt.title('[A]stack_%d_%s_band' % (N_sum[tt], band[tt]) )
			...
			plt.xlim(x0 - 2 * Rpp, x0 + 2 * Rpp)
			plt.ylim(y0 - 2 * Rpp, y0 + 2 * Rpp)
			plt.savefig(home + 'fig_15sigma/stack_img/A_mask/A_stack_%d_%s_band.png' % (N_sum[tt], band[tt]), dpi = 300)
			plt.close()

			fig = plt.figure()
			ax = plt.subplot(111)
			ax.set_title('[A mask]SB profile %d %s band' % (N_sum[tt], band[tt]) )
			...
			plt.savefig(home + 'fig_15sigma/stack_img/A_mask/A_stack_%d_%s_band_SB.png' % (N_sum[tt], band[tt]), dpi = 300)
			plt.close()
	...
	if rank == 1:
		for tt in range(3):
			...
			for pp in range(cpus):
				...
				with h5py.File(tmp + 'stack_Bmask_pcount_%d_in_%s_band.h5' % (pp, band[tt]), 'r')as f:
					p_count = np.array(f['a'])
				with h5py.File(tmp + 'stack_Bmask_sum_%d_in_%s_band.h5' % (pp, band[tt]), 'r') as f:
					sum_img = np.array(f['a'])
				...
				plt.figure()
				ax = plt.subplot(111)
				ax.set_title('B mask %s band %d cpus %d imgs' % (band[tt], pp, sub_Num) )
				ax = plt.imshow(sub_mean, cmap = 'Greys', vmin = 1e-7, vmax = 5e1, origin = 'lower', norm = mpl.colors.LogNorm())
				plt.colorbar(ax, fraction = 0.035, pad =  0.01, label = '$flux[nmaggy]$')
				hsc.circles(x0, y0, s = Rpp, fc = '', ec = 'r', linestyle = '-', alpha = 0.5)
				hsc.circles(x0, y0, s = 0.2 * Rpp, fc = '', ec = 'g', linestyle = '--', alpha = 0.5)	
				plt.xlim(x0 - 2 * Rpp, x0 + 2 * Rpp)
				plt.ylim(y0 - 2 * Rpp, y0 + 2 * Rpp)
				plt.savefig(home + 'fig_15sigma/stack_img/B_mask/B_sub_stack_%d_cpus_%s_band.png' % (pp, band[tt]), dpi = 300)
				plt.close()
			...
			with h5py.File(home + 'fig_15sigma/stack_img/B_mask/stack_Bmask_%d_in_%s_band.h5' % (tot_N, band[tt]), 'w') as f:
				f['a'] = np.array(stack_img)
	commd.Barrier()
	...
	if rank == 0:
		for tt in range(3):
			...
			with h5py.File(home + 'fig_15sigma/stack_img/B_mask/stack_Bmask_%d_in_%s_band.h5' % (N_sum[tt], band[tt]), 'r') as f:
				stack_img = np.array(f['a'])
			...
			plt.figure()
			plt.title('[B]stack_%d_%s_band' % (N_sum[tt], band[tt]) )
			ax = plt.imshow(stack_img, cmap = 'Greys', vmin = 1e-7, vmax = 5e1, origin = 'lower', norm = mpl.colors.LogNorm())
			plt.colorbar(ax, fraction = 0.035, pad =  0.01, label = '$flux[nmaggy]$')
			hsc.circles(x0, y0, s = Rpp, fc = '', ec = 'r', linestyle = '-', alpha = 0.5)
			hsc.circles(x0, y0, s = 0.2 * Rpp, fc = '', ec = 'g', linestyle = '--', alpha = 0.5)	
			plt.xlim(x0 - 2 * Rpp, x0 + 2 * Rpp)
			plt.ylim(y0 - 2 * Rpp, y0 + 2 * Rpp)
			plt.savefig(home + 'fig_15sigma/stack_img/B_mask/B_stack_%d_%s_band.png' % (N_sum[tt], band[tt]), dpi = 300)
			plt.close()

			fig = plt.figure()
			ax = plt.subplot(111)
			ax.set_title('[B mask]SB profile %d %s band' % (N_sum[tt], band[tt]) )
			ax.errorbar(Rt, SBt, yerr = [t_err0, t_err1], xerr = None, color = 'r', marker = '.', ls = '', linewidth = 1, markersize = 5, 
				ecolor = 'r', elinewidth = 1, label = 'stack image', alpha = 0.5)
			ax.plot(R_obs, SB_obs, 'b:', label = 'Z05', alpha = 0.5)
			ax.plot(Intns_r, SB_Z05, 'k--', label = 'Sersic Z05', alpha = 0.5)
			...
			ax.grid(which = 'both', axis = 'both')
			ax.tick_params(axis = 'both', which = 'both', direction = 'in')
			plt.savefig(home + 'fig_15sigma/stack_img/B_mask/B_stack_%d_%s_band_SB.png' % (N_sum[tt], band[tt]), dpi = 300)
			plt.close()
