...
for k in range(len(z)):
    for q in range(len(band)):
        ...
        plt.savefig('/mnt/ddnfs/data_users/cxkttwl/ICL/select_%s_ra%.3f_dec%.3f_z%.3f_rich%.3f.png'%(band[q],ra[k],dec[k],z[k],rich[k]),dpi = 600)  
        plt.close()
        ...
        fil = fits.Header(ff)
        fits.writeto(
                '/mnt/ddnfs/data_users/cxkttwl/ICL/cut_image_%s_ra%.3f_dec%.3f_z%.3f_rich%.3f.fits'%(
        ...
        plt.savefig('/mnt/ddnfs/data_users/cxkttwl/ICL/resamp_%s_ra%.3f_dec%.3f_z%.3f_rich%.3f.png'%(band[q],ra[k],dec[k],z[k],rich[k]),dpi = 600)  
        plt.close()
        ...
        fits.writeto(
                '/mnt/ddnfs/data_users/cxkttwl/ICL/resamp_image_%s_ra%.3f_dec%.3f_z%.3f_rich%.3f.fits'%(
                band[q],ra[k],dec[k],z[k],rich[k]), mirro, header = fil, overwrite=True) 
