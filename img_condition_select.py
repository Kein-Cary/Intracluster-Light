...
np.random.seed( 6 )
tt0 = np.random.choice(Nz, size = Nt, replace = False)
'''
...
set_z, set_ra, set_dec = z[tt0], ra[tt0], dec[tt0] ## random

S0 = []
binx = np.arange(0, 2048, 200)
binx = np.r_[binx, 2048]
biny = np.arange(0, 1489, 200)
biny = np.r_[biny, 1489]
grdx = len(binx)
grdy = len(biny)
...
e_crit = 0.85
rho_crit = 6.5e-4 ## for bins ~ 100^2 pixel^2
dR_cen = 950 ## about 1Mpc / h to the cluster center (assuming z ~ 0.25)
cen_crit = 50 ## avoid choose BCG as a 'bad-feature'
...
for kk in range( len(set_z) ):
	...
	id_R = (R_cen > cen_crit) & (R_cen <= dR_cen)
...
'''
	if np.sum(id_big) > 0: ## too big source
		...
		bx.set_title('Number density [bins = %d * %d]' % (grdy - 1, grdx - 1) )
		...
		idu = densi > rho_crit
		iy, ix = np.where(idu == True)
		for mm in range( np.sum(idu) ):
			a0, a1 = binx[ ix[mm] ], binx[ ix[mm] + 1 ]
			b0, b1 = biny[ iy[mm] ], biny[ iy[mm] + 1 ]
...
plt.figure()
ax = plt.subplot(111)
ax.hist(S0, bins = 15, alpha = 0.5)
plt.savefig('sample_max-S.png', dpi = 300)
plt.close()

with h5py.File('test_sample.h5', 'w') as f:
	f['area'] = np.array(S0)
	f['ra'] = np.array(set_ra)
	f['dec'] = np.array(set_dec)
	f['z'] = np.array(set_z)
'''
