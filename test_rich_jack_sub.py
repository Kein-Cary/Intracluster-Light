...
from Mass_rich_radius import rich2R_Simet, rich2R_critical
...
def main():
	...
	for kk in range(3):
		...
		M_vir_c, R_vir_c = rich2R_critical(set_z, set_rich)
		M_vir_m, R_vir_m = rich2R_Simet(set_z, set_rich)
	...
	for kk in range(rank, rank + 1):
		for lamda_k in range(3):
			for nn in range(N_bin):
				...
				bins_0 = np.int( np.ceil(bin_1Mpc * bl_set * m_R200 / 1e3) )
				R_min_0, R_max_0 = 1, bl_set * m_R200 # kpc
				R_min_1, R_max_1 = bl_set * m_R200 + 100., R_max # kpc
