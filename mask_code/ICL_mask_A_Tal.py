...
def mask_A():
	...
	out_load_A = './result/mask_A_Tal.cat'
	...
	for q in range(stack_N):
		...
		for l in range(len(band_fil)):
			...
			hdu.data = combine
			hdu.header = head_inf
			hdu.writeto('combine_data_tal.fits', overwrite = True)
			file_source = './combine_data_tal.fits'
			...
			hdu.data = mirro_A
			hdu.header = head_inf
			hdu.writeto(load + 'mask_data/A_plane/Tal/A_mask_data_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band_fil[l], ra_g, dec_g, z_g),overwrite = True)

			hdu = fits.PrimaryHDU()
			hdu.data = mask_A
			hdu.header = head_inf
			hdu.writeto(load + 'mask_metrx/mask_A/Tal/A_mask_metrx_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band_fil[l], ra_g, dec_g, z_g),overwrite = True)
			...
			plt.xlim(0, 2048)
			plt.ylim(0, 1489)
			plt.savefig('/mnt/ddnfs/data_users/cxkttwl/ICL/fig_cut/region_cut/mask_A_Tal/A_mask_%s_ra%.3f_dec%.3f_z%.3f.png'%(band_fil[l], ra[q], dec[q], z[q]), dpi = 300)
			plt.close()
