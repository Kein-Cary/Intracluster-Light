...
def mask_A():
	...
	out_load_A = './result/mask_A_15.cat'
	...
	for q in range(stack_N):
		...
		for l in range(len(band_fil)):
			...
			mask = load + 'mask_catalog/source_SQL_Z%.3f_ra%.3f_dec%.3f.txt'%(z_g, ra_g, dec_g)
			...
			hdu.data = mirro_A
			hdu.header = head_inf
			hdu.writeto(load + 'mask_data/A_plane/1.5sigma/A_mask_data_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band_fil[l], ra_g, dec_g, z_g),overwrite = True)
			...
			plt.title('A mask img ra%.3f dec%.3f z%.3f in %s band' % (ra_g, dec_g, z_g, band_fil[l]))
			plt.xlim(0, mirro_A.shape[1])
			plt.ylim(0, mirro_A.shape[0])
			plt.savefig(
				'/mnt/ddnfs/data_users/cxkttwl/ICL/fig_class/A_mask/A_mask_%s_ra%.3f_dec%.3f_z%.3f.png'%(band_fil[l], ra_g, dec_g, z_g), dpi = 300)
			plt.close()
