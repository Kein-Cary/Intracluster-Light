...
def stack_light():
    ...
    for ii in range(len(band)):
        ...
        for jj in range(stack_N):
            ...
            data = fits.getdata(load + 'mask_data/A_plane/zib_tal/A_mask_data_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band[ii], ra_g, dec_g, z_g), header = True)
            img = data[0]
            wcs = awc.WCS(data[1])
        ...
        '''
        #staack sky for 0.3sigma threshold
        ...
        for kk in range(stack_N):
            ...
            z_g = z[kk]
            Da_g = Test_model.angular_diameter_distance(z_g).value
            data = fits.getdata(load + 'mask_data/sky_plane/0.3sigma/sky_mask_data_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band[ii], ra_g, dec_g, z_g), header = True)
            img = data[0]
            wcs = awc.WCS(data[1])
            cx, cy = wcs.all_world2pix(ra_g*U.deg, dec_g*U.deg, 1)
        ...
        ax1.invert_yaxis()
        plt.savefig('/mnt/ddnfs/data_users/cxkttwl/ICL/fig_cut/stacking_0.3sigma_%s.png'%band[ii], dpi = 600)
        plt.close()
