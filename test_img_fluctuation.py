...
N_bin = 30
d_load = load + 'rich_sample/scale_fig/R200_c/'
...
for kk in range(rank, rank + 1):
	...
	for lamda_k in range(2,3):
		...
		with h5py.File( load + 'rich_sample/jackknife/%s_band_%d_rich_R200c.h5' % (band[kk], lamda_k), 'r') as f:
		...
		for nn in range(N_bin):
			...
			with h5py.File(d_load + '%s_band_%d_rich_%d_sub-stack_sky_ICL.h5' % (band[kk], lamda_k, nn), 'r') as f:
				dmp_array = np.array(f['a'])	for kk in range(3):
		...
		#ax0.plot(Add_R, Add_SB, ls = '-', color = 'k', alpha = 0.5, label = 'Mean of SB profile')
		#ax0.plot(jk_cli_R, jk_cli_SB, ls = '-', color = 'k', alpha = 0.5, label = 'Mean of SB profile')
		...
		ax0.axvline(x = R200, color = 'k', alpha = 0.5, linestyle = '--', label = 'Median of R200c %.1f Mpc' % R200 )
		...
		#ax0.set_ylim(1e-3, 1e-2)
		ax0.legend(loc = 1, frameon = False)
		#ax0.grid(which = 'both', axis = 'both')
		ax0.tick_params(axis = 'both', which = 'both', direction = 'in')

		#ax1.plot(jk_add_R, jk_add_SB, ls = '-', color = 'k', alpha = 0.5, label = 'Mean of SB profile')
		#ax1.plot(JK_R, JK_SB, ls = '-', color = 'k', alpha = 0.5, label = 'Mean of SB profile')
		ax1.errorbar(JK_R, JK_SB, yerr = [JK_err0, JK_err1], xerr = None, color = 'k', marker = 'None', 
			ls = '-', linewidth = 1, ecolor = 'k', elinewidth = 1, alpha = 0.5, label = 'Jackknife SB')		
		ax1.axvline(x = R200, color = 'k', alpha = 0.5, linestyle = '--', label = 'Median of R200c %.1f Mpc' % R200 )
