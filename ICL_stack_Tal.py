...
def stack_light():
    ...
    for ii in range(len(band)):
        ...
        for jj in range(stack_N):
            ...
            data = fits.getdata(load + 'mask_data/A_plane/Tal/A_mask_data_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band[ii], ra_g, dec_g, z_g), header = True)
            img = data[0]
            wcs = awc.WCS(data[1])
        ...
        ax1.invert_yaxis()
        plt.savefig('/mnt/ddnfs/data_users/cxkttwl/ICL/fig_cut/stack_img/stacking_Tal_%s.png'%band[ii], dpi = 600)
        plt.close()
