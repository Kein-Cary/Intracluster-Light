...
for dd in range( 2 ):
	...
	for ll in range( len( R_bins ) - 1 ):
		...
		for kk in range( 3 ):
			...
			dat = pds.read_csv( cat_path + '%s_clust_frame-lim_Pm-cut_%.2f-%.2fR200m_mem_%s-band_pos-zref.csv'
			...
			# XXX
			sub_img = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_sub-%d_img.h5'
			sub_pix_cont = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_sub-%d_pix-cont.h5'
			sub_sb = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_sub-%d_SB-pro.h5'
			# XXX

			J_sub_img = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_jack-sub-%d_img_z-ref.h5'
			J_sub_pix_cont = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_jack-sub-%d_pix-cont_z-ref.h5'
			J_sub_sb = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_jack-sub-%d_SB-pro_z-ref.h5'

			jack_SB_file = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_Mean_jack_SB-pro_z-ref.h5'
			jack_img = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_Mean_jack_img_z-ref.h5'
			jack_cont_arr = out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + '_Mean_jack_pix-cont_z-ref.h5'
...
##. BG_img stacking
img_path = '/home/xkchen/data/SDSS/member_files/Mrich_binned_shufl_img/align_frame/sat_woBCG/resamp_img/'
d_file = img_path + 'Sat-tract_%s-band_clus_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_resamp-img.fits'
	...
	for ll in range( len( R_bins ) - 1 ):
		...
		for kk in range( 3 ):
			...
			dat = pds.read_csv( cat_path + 
						'%s_clust_frame-lim_Pm-cut_%.2f-%.2fR200m_mem_%s-band_pos-zref.csv'
						% (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str),)
			...
			# XXX
			sub_img = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_sub-%d_img.h5',)[0]

			sub_pix_cont = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_sub-%d_pix-cont.h5',)[0]

			sub_sb = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_sub-%d_SB-pro.h5',)[0]

			# XXX
			J_sub_img = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_jack-sub-%d_img_z-ref.h5',)[0]

			J_sub_pix_cont = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_jack-sub-%d_pix-cont_z-ref.h5',)[0]

			J_sub_sb = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_jack-sub-%d_SB-pro_z-ref.h5',)[0]

			jack_SB_file = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_Mean_jack_SB-pro_z-ref.h5',)[0]

			jack_img = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
						'_shufl-%d_BG' % list_order + '_Mean_jack_img_z-ref.h5',)[0]

			jack_cont_arr = ( out_path + '%s_clust_%.2f-%.2fR200m_%s-band' % (cat_lis[dd], R_bins[ll], R_bins[ll+1], band_str) + 
