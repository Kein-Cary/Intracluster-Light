...
shufl_path = '/home/xkchen/Pictures/BG_calib_SBs/BCG_M_bin/shufl_list/align_frame/'
...
for dd in range( 2 ):
	...
	for pp in range( 3 ):
		...
		for tt in range( len(R_bins) - 1 ):
			...
			sub_cat_file = ( out_path + '%s_clust_frame-lim_Pm-cut_rich_%d-%d_phyR_%d-%dkpc_mem_cat.csv' % 
			...
			for kk in range( 3 ):
				...
				shufl_list_file = ( shufl_path + 
							...
							% (cat_lis[dd], bin_rich[pp], bin_rich[pp + 1], band_str, tag_dex), )[0]

				out_shufl_file = ( out_path + 
							'%s_clust_frame-lim_Pm-cut_rich_%d-%d_phyR_%d-%dkpc_mem_%s-band_sat_fixRs-shufl-%d_cat.csv' 
							% (cat_lis[dd], bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str, tag_dex), )[0]
