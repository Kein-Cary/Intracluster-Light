...
for ll in range( 3 ):
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        # I0 = integ.simpson( pm_F_halo, x_mm )
        # I1 = integ.simpson( pm_F_weit, x_mm )
        ...
        pp_alpha_k.append( alpha_k )

        ##.
        ax0.plot( r_bins, aveg_rho_sat, ls = '-', color = color_s[tt], alpha = 0.75, label = fig_name[tt],)

        ax0.axhline( y = np.abs( alpha_k ) * tag_BG, ls = ':', color = color_s[tt],)
...
for qq in range( 3 ):
    ...
    cc_dat = pds.read_csv( '/home/xkchen/figs/extend_bcgM_cat_Sat/rich_R_rebin/nobcg_BGsub_SBs/' + 
            'Extend_BCGM_gri-common_%s_%s_r-band_Rt_test.csv' % (sub_name[qq], R_str),)
    ...
    cp_Rc  = np.array( cp_Rc ) * h  ##. kpc / h


    ##. estimate with ratio slope
    cc_Rc = []
    ...
    for dd in range( len(R_bins) - 2 ):
        ...
        dat = pds.read_csv( '/home/xkchen/figs/extend_bcgM_cat_Sat/rich_R_rebin/nobcg_BGsub_SBs/'
                ...
                + '_r-band_aveg-jack_BG-sub_SB_ratio_slope.csv',)

        tt_Rc = np.array( dat['R_crit'] )[0]
        cc_Rc.append( tt_Rc )

    cc_Rc  = np.array( cc_Rc ) * h  ##. kpc / h


    ax1.plot( R_aveg, Rc, marker = marks[qq], ls = '-', color = color_s[qq], label = line_name[qq], alpha = 0.75,)

    # if qq == 0:
    #     ax1.plot( R_aveg[:4], cc_Rc * 5, marker = marks[qq], ls = '--', color = color_s[qq], alpha = 0.45, label = 'Min-slope')
    # else:
    #     ax1.plot( R_aveg[:4], cc_Rc * 5, marker = marks[qq], ls = '--', color = color_s[qq], alpha = 0.45,)

    if qq == 0:
        ax1.plot( R_aveg[:4], cp_Rc * 5, marker = marks[qq], ls = '--', color = color_s[qq], alpha = 0.45, label = 'Ratio decrease')

    else:
        ax1.plot( R_aveg[:4], cp_Rc * 5, marker = marks[qq], ls = '--', color = color_s[qq], alpha = 0.45,)
