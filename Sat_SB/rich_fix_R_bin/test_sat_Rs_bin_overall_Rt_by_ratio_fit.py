...
for kk in range( 1 ):
	...
	tab_file.write( out_path + 
				'Extend_BCGM_gri-common_Rs-bin_over-rich_%s-band_polyfit_Rt_test.fits' % band_str, overwrite = True)
	...
	out_data.to_csv( out_path + 
				'Extend_BCGM_gri-common_Rs-bin_over-rich_%s-band_polyfit_params.csv' % band_str,)
...
for kk in range( 1 ):
	...
	for oo in range( len(crit_eta) ):
		...
		##.
		pat = fits.open( out_path + 'Extend_BCGM_gri-common_Rs-bin_over-rich_%s-band_polyfit_Rt_test.fits' % band_str,)
		p_table = pat[1].data
		...
		##.
		pat = pds.read_csv( out_path + 'Extend_BCGM_gri-common_Rs-bin_over-rich_%s-band_polyfit_params.csv' % band_str,)
		...
		for mm in range( len(R_bins) -2 ):
			...
			##.
			gax.plot( nbg_R[mm], _dd_eta, ls = '-', color = color_s[mm], alpha = 0.75, label = fig_name[mm],)

			gax.fill_between( nbg_R[mm], y1 = _dd_eta - nbg_err[mm] / _kk_tmp_F( nbg_R[mm] ), 
						y2 = _dd_eta + nbg_err[mm] / _kk_tmp_F( nbg_R[mm] ), color = color_s[mm], alpha = 0.12,)
		...
		#.
		gax.annotate( s = '%s-band' % band_str, xy = (0.52, 0.87), xycoords = 'axes fraction', fontsize = 14, backgroundcolor = 'w',)
