...
def Rs_bin_over_rich():
	...
	plt.savefig('/home/xkchen/contrl-galx_%s-band_BG-sub_SB_compare_1.png' % band_str, dpi = 300)
	plt.close()
...
def rich_bin_Rs_bin():
	...
	for tt in range( len(R_bins) - 1 ):
		...
		dat = pds.read_csv( cp_out_path + 'Extend_BCGM_gri-common_%s_%.2f-%.2fR200m_%s-band_aveg-jack_BG-sub_SB.csv' % 
							(sub_name[ ll ], R_bins[tt], R_bins[tt + 1], band_str), )
	...
	for mm in range( len(R_bins) - 1 ):
		...
		l2 = ax1.errorbar( nbg_R[mm], nbg_R[mm] * nbg_SB[mm], yerr = nbg_R[mm] * nbg_err[mm], marker = '.', ls = '--', color = color_s[mm],
			ecolor = color_s[mm], mfc = 'none', mec = color_s[mm], capsize = 1.5, alpha = 0.75,)

		l3 = ax1.errorbar( cp_nbg_R[mm], cp_nbg_R[mm] * cp_nbg_SB[mm], yerr = cp_nbg_R[mm] * cp_nbg_err[mm], marker = '.', ls = '-', color = color_s[mm],
			ecolor = color_s[mm], mfc = 'none', mec = color_s[mm], capsize = 1.5, alpha = 0.75, label = fig_name[mm],)
	...
	ax1.annotate( s = '%s, %s-band' % (line_name[ll], band_str), xy = (0.03, 0.48), xycoords = 'axes fraction', fontsize = 12,)

	# ax1.set_ylim( 2e-3, 5e0 )
	# ax1.set_ylabel('$ \\mu \; [nanomaggy \, / \, arcsec^{2}]$', fontsize = 12,)

	ax1.set_ylim( 8e-2, 8e0 )
	ax1.set_ylabel('$ R \\times \\mu \; [nanomaggy \, / \, arcsec^{2}]$', fontsize = 12,)
	ax1.set_yscale('log')
	...
	plt.savefig('/home/xkchen/contrl-galx_%s_%s-band_BG-sub_SB_compare.png' % (sub_name[ll], band_str), dpi = 300)
	plt.close()
