"""
randomly located satellite among cluster but fixed centric distance of satellite only
"fixRs-shufl"
"""
...
out_path = '/home/xkchen/figs/extend_bcgM_cat_Sat/shufl_fixRsat_only/map_cat/'
shufl_path = '/home/xkchen/figs/extend_bcgM_cat_Sat/shufl_fixRsat_only/shufl_cat/'
...
for pp in range( 3 ):
	...
	for tt in range( len(R_bins) - 1 ):
		...
		for kk in range( 1 ):
			...
			##.
			out_shufl_file = ( out_path + 'Extend-BCGM_rgi-common_frame-lim_Pm-cut_rich_%d-%d_%.2f-%.2fR200m_mem_%s-band_' 
								% ( bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str) + 'sat-fixRsat-shufl-%d_cat.csv',)[0]

			out_pos_file = ( out_path + 'Extend-BCGM_rgi-common_frame-lim_Pm-cut_rich_%d-%d_%.2f-%.2fR200m_mem_%s-band_'
								% ( bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str) + 'sat-fixRsat-shufl-%d_origin-img_position.csv',)[0]

			out_Ng_file = ( out_path + 'Extend-BCGM_rgi-common_frame-lim_Pm-cut_rich_%d-%d_%.2f-%.2fR200m_mem_%s-band_' 
								% ( bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str) + 'sat-fixRsat-shufl-%d_shufl-Ng.csv',)[0]
