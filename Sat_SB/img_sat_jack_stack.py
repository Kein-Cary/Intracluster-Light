...
def jack_main_func(id_cen, N_bin, n_rbins, cat_ra, cat_dec, cat_z, sat_ra, sat_dec, img_x, img_y, img_file, band_str, sub_img,
	...
	for nn in range(N_bin):
		...
		if id_cut == False:
			stack_func( img_file, sub_img_file, set_z, set_ra, set_dec, band[ band_id ], set_s_ra, set_s_dec, set_x, set_y, id_cen,
				rms_file = sub_rms_file, pix_con_file = sub_cont_file,)
		if id_cut == True:
			cut_stack_func( img_file, sub_img_file, set_z, set_ra, set_dec, band[ band_id ], set_s_ra, set_s_dec, set_x, set_y, id_cen, N_edg,
				rms_file = sub_rms_file, pix_con_file = sub_cont_file,)
