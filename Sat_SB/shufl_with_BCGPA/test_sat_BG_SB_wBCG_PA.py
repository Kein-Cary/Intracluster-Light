...
### === ### satellite image extract without BCG
def cut_woBCG():
	...
	for tt in range( len(R_bins) - 1 ):
		...
		for kk in range( 3 ):
			...
			bg_sb_file = ( BG_path + 
						...
						(R_bins[tt], R_bins[tt + 1], band_str, list_order),)[0]
	...
	for tt in range( len(R_bins) - 1 ):
		...
		for kk in range( 3 ):
			...
			with h5py.File( BG_path + 'Sat-all_%.2f-%.2fR200m_%s-band_wBCG-PA_shufl-%d_BG_Mean_jack_SB-pro_z-ref.h5' % 
	...
	for kk in range( 3 ):
		...
		legend_2 = ax1.legend( handles = [ l2, l3], labels = [ 'Align with BCG (AB)', 'Align with frame (AF)' ], 
		...
		sub_ax1.set_ylabel('$\\mu_{AF} \; / \; \\mu_{AB}$', labelpad = 10, fontsize = 12,)
		...
		plt.savefig('/home/xkchen/galx_%s-band_BG-sub_SB_compare.png' % band_str, dpi = 300)
		plt.close()
...
### === ### satellite image extract with BCG
def cut_wBCG():
	...
	for tt in range( len(R_bins) - 1 ):
		...
		for kk in range( 1 ):
			...
			with h5py.File( BG_path + 'Sat-all_%.2f-%.2fR200m_%s-band_wBCG-PA_shufl-%d_BG-wBCG_Mean_jack_SB-pro_z-ref.h5' % 
						(R_bins[tt], R_bins[tt + 1], band_str, list_order), 'r') as f:
