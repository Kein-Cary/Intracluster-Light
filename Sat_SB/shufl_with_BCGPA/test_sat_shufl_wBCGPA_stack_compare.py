...
##.. the satellite image extract with BCGs~( position angle is relative to the BCG major axis)
...
##. sat_img with BCG
img_path = '/home/xkchen/data/SDSS/member_files/shufl_wBCG_wPA/resamp_img/'
...
for kk in range( 1 ):
	...
	for tt in range( 3 ):
		...
		sub_img = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_sub-%d_img.h5',)[0]

		sub_pix_cont = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_sub-%d_pix-cont.h5',)[0]

		sub_sb = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_sub-%d_SB-pro.h5',)[0]

		# XXX
		J_sub_img = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_jack-sub-%d_img_z-ref.h5',)[0]

		J_sub_pix_cont = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_jack-sub-%d_pix-cont_z-ref.h5',)[0]

		J_sub_sb = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_jack-sub-%d_SB-pro_z-ref.h5',)[0]

		jack_SB_file = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_Mean_jack_SB-pro_z-ref.h5',)[0]

		jack_img = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_Mean_jack_img_z-ref.h5',)[0]

		jack_cont_arr = ( out_path + 'Sat-all_%.2f-%.2fR200m' % (R_bins[tt], R_bins[tt + 1]) + 
					'_%s-band_wBCG-PA_shufl-%d_BG-wBCG' % (band_str, list_order) + '_Mean_jack_pix-cont_z-ref.h5',)[0]
