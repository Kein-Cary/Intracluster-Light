...
def sate_Extract_func( d_file, bcg_ra, bcg_dec, bcg_z, ra_set, dec_set, z_set, band, gal_file, out_file, R_cut, 
	...
	for pp in range( N_sat ):
		...
		#. record the object center in the cutout image
		dL = kk_major_R * R_cut
		...
		cut_img = img_arr[ d_y0 : d_y1, d_x0 : d_x1 ]
		...
		#. peak position record (in the cutout image)
		set_r = np.int(np.ceil( 1.5 * cen_ar) )
		la0 = np.max( [np.int( kk_px - set_r), 0])
		la1 = np.min( [np.int( kk_px + set_r + 1), img_arr.shape[1] ] )

		lb0 = np.max( [np.int( kk_py - set_r), 0] ) 
		lb1 = np.min( [np.int( kk_py + set_r + 1), img_arr.shape[0] ] )

		_tag_flux = img_arr[ lb0 : lb1, la0 : la1 ]

		id_vx = np.where( _tag_flux == np.nanmax( _tag_flux ) )

		_pkx, _pky = id_vx[1][0] + la0, id_vx[0][0] + lb0
...
def sate_surround_mask_func(d_file, cat_file, bcg_ra, bcg_dec, bcg_z, ra_set, dec_set, z_set, band, gal_file, out_mask_file, R_cut, 
	...
	R_cut : 0.5 width of cut region, in units of Kron radius or radius in sdss_photo_file, default is 5.
	...
	## ... satellite galaxy region (cutout satellite images with surrounding mask)
	s_xn, s_yn = WCS_to_pixel_func( ra_set, dec_set, Header )
	s_xn, s_yn = s_xn + medi_x2pk_off, s_yn + medi_y2pk_off
	...
	for pp in range( N_sat ):
		...
		cen_ar = A[ id_order ] * 5 # 1.5
		cen_br = B[ id_order ] * 5 # 1.5
		...
		#. record the center of target galaxy in the cutout image
		if kk_px - dL <= 0:
			cc_px = kk_px - dL + dL
		if kk_px - dL > 0:
			cc_px = dL + 0.
		...
		#. galaxy mask
		pre_mask_img = mask_with_G_tag( cut_img, cc_px, cc_py, cen_ar, cen_br, cen_cr, cen_chi, lim_gal_arr )
		...
		#. masking stars
		mask_path = np.ones( (cut_img.shape[0], cut_img.shape[1]), dtype = np.float32)
		ox = np.linspace( 0, cut_img.shape[1] - 1, cut_img.shape[1] )
		oy = np.linspace( 0, cut_img.shape[0] - 1, cut_img.shape[0] )
		...
		for k in range( lim_Ns ):
			...
			la0 = np.max( [np.int(xc - set_r), 0])
			la1 = np.min( [np.int(xc + set_r + 1), cut_img.shape[1] ] )
			lb0 = np.max( [np.int(yc - set_r), 0] ) 
			lb1 = np.min( [np.int(yc + set_r + 1), cut_img.shape[0] ] )
		...
		ff = dict(zip(keys,value))
		fil = fits.Header(ff)
		fits.writeto( out_mask_file % (band, bcg_ra, bcg_dec, bcg_z, kk_ra, kk_dec ), mask_img, header = fil, overwrite = True)
