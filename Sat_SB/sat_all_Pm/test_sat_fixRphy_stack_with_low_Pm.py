...
for ll in range( 3 ):
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        for kk in range( 3 ):
            ...
            # XXX
            sub_img = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_sub-%d_img.h5',)[0]
            sub_pix_cont = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_sub-%d_pix-cont.h5',)[0]
            sub_sb = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_sub-%d_SB-pro.h5',)[0]
            # XXX

            J_sub_img = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_jack-sub-%d_img_z-ref.h5',)[0]
            J_sub_pix_cont = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_jack-sub-%d_pix-cont_z-ref.h5',)[0]
            J_sub_sb = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_jack-sub-%d_SB-pro_z-ref.h5',)[0]

            jack_SB_file = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_Mean_jack_SB-pro_z-ref.h5',)[0]
            jack_img = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                        '_%s-band' % band_str + '_Mean_jack_img_z-ref.h5',)[0]
            jack_cont_arr = ( out_path + 'clus-sat_with-low-Pm_%s_%d-%dkpc' % (sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        for kk in range( 1 ):
            ...
            cc_pat = pds.read_csv( cat_path + 
                    'clust_rich_%d-%d_rgi-common_frame-lim_lower-Pm_phyR_%d-%dkpc_mem_%s-band_sat-shufl-%d_shufl-Ng.csv' 
                    % ( bin_rich[ ll ], bin_rich[ll + 1], R_bins[tt], R_bins[tt + 1], band_str, list_order),)
