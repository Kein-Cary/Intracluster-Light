...
for ll in range( 3 ):
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        for kk in range( 1 ):
            ...
            ##.
            sat_sb_file = ( path + 'clus-sat_with-low-Pm_%s_%.2f-%.2fR200m' % (sub_name[ll], R_bins[tt], R_bins[tt + 1]) + 
                            '_%s-band' % band_str + '_jack-sub-%d_SB-pro_z-ref.h5',)[0]

            bg_sb_file = ( BG_path + 'clus-sat_with-low-Pm_%s_%.2f-%.2fR200m' % (sub_name[ll], R_bins[tt], R_bins[tt + 1]) + 
                            '_%s-band_shufl-%d_BG' % (band_str, list_order) + '_Mean_jack_SB-pro_z-ref.h5',)[0]

            sub_out_file = ( out_path + 'clus-sat_with-low-Pm_%s_%.2f-%.2fR200m' % (sub_name[ll], R_bins[tt], R_bins[tt + 1]) + 
                            '_%s-band' % band_str + '_jack-sub-%d_BG-sub-SB-pro_z-ref.h5',)[0]

            out_file = ( out_path + 'clus-sat_with-low-Pm_%s_%.2f-%.2fR200m' % (sub_name[ll], R_bins[tt], R_bins[tt + 1]) + 
...
for tt in range( len(R_bins) - 1 ):
    ...
    for kk in range( 1 ):
        ...
        with h5py.File( path + 'clus-sat_with-low-Pm_%s_%.2f-%.2fR200m' % (sub_name[ll], R_bins[tt], R_bins[tt + 1]) 
...
for tt in range( len(R_bins) - 1 ):
    ...
    for kk in range( 1 ):
        ...
        with h5py.File( BG_path + 'clus-sat_with-low-Pm_%s_%.2f-%.2fR200m' % (sub_name[ll], R_bins[tt], R_bins[tt + 1]) 
