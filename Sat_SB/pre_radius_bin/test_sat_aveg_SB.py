...
### === ### data load
path = '/home/xkchen/figs/extend_bcgM_cat_Sat/Radii_bin/SBs/'
BG_path = '/home/xkchen/figs/extend_bcgM_cat_Sat/Radii_bin/BGs/'
out_path = '/home/xkchen/figs/extend_bcgM_cat_Sat/Radii_bin/nBG_SBs/'
...
for mm in range( 2 ):
	...
	for kk in range( 3 ):
		...
		with h5py.File( BG_path + 'Extend_BCGM_gri-common_%s_%s-band_BG_Mean_jack_SB-pro_z-ref.h5' % (cat_lis[mm], band[kk]), 'r') as f:
...
for mm in range( 2 ):
	...
	for kk in range( 3 ):
		...
		sat_sb_file = path + 'Extend_BCGM_gri-common_%s_%s-band_' % (cat_lis[mm], band[kk]) + 'jack-sub-%d_SB-pro_z-ref.h5'
		bg_sb_file = BG_path + 'Extend_BCGM_gri-common_%s_%s-band_BG_Mean_jack_SB-pro_z-ref.h5' % (cat_lis[mm], band[kk])
		out_file = out_path + 'Extend_BCGM_gri-common_%s_%s-band_aveg-jack_BG-sub_SB.csv' % (cat_lis[mm], band[kk])
...
for mm in range( 2 ):
	...
	for kk in range( 3 ):
		...
		dat = pds.read_csv( out_path + 'Extend_BCGM_gri-common_%s_%s-band_aveg-jack_BG-sub_SB.csv' % (cat_lis[mm], band[kk]),)
...
legend_2 = ax1.legend( [ fig_name[0], fig_name[1] ], loc = 3, frameon = False,)
...
for kk in range( 3 ):
	...
	_kk_tmp_F = interp.interp1d( nbg_R[1][kk], nbg_SB[1][kk], kind = 'linear', fill_value = 'extrapolate')
	...
	sub_ax1.fill_between( nbg_R[1][kk], y1 = (nbg_SB[1][kk] - nbg_err[1][kk]) / nbg_SB[1][kk], 
				y2 = (nbg_SB[1][kk] + nbg_err[1][kk]) / nbg_SB[1][kk], color = color_s[kk], alpha = 0.12,)
