...
from img_partial_cut_stack import cut_stack_func, stack_func
...
def jack_main_func(id_cen, N_bin, n_rbins, cat_ra, cat_dec, cat_z, img_x, img_y, img_file, band_str, sub_img,
	...
	bcg_PA, tag_x, tag_y, id_half = 'half', sat_PA = None, sat_ar = None, 
	id_cut = False, N_edg = None, id_Z0 = True, z_ref = None, id_sub = True, 
	edg_bins = None, sub_rms = None, J_sub_rms = None, jack_rms_arr = None ):
	"""
	...
	N_edg : the cut region width, in unit of pixel, only applied when id_cut == True, pixels in this region will be
			set as 'no flux' contribution pixels (ie. set as np.nan)
	...
	if id_half == 'angle':
		...
		for nn in range( N_bin ):
			...
			lis_PA.append( bcg_PA[ id_xbin ] )

			lis_sat_x.append( tag_x[ id_xbin ] )
			lis_sat_y.append( tag_y[ id_xbin ] )

			lis_sat_PA.append( sat_PA[ id_xbin ] )
			lis_sat_ar.append( sat_ar[ id_xbin ] )
		...
		for nn in range(N_bin):
			...
			set_sat_x = lis_sat_x[ nn ]
			set_sat_y = lis_sat_y[ nn ]

			set_PAs = lis_PA[ nn ]

			set_sat_PA = lis_sat_PA[ nn ]
			set_sat_ar = lis_sat_ar[ nn ]
			...
			if id_cut == False:
				stack_func(img_file, sub_img_file, set_z, set_ra, set_dec, band_str, set_x, set_y, id_cen,
					set_PAs, set_sat_x, set_sat_y, id_half = id_half, sat_PA = set_sat_PA, sat_ar = set_sat_ar,
	...
	### === Half-mask BCG case
		...
		for nn in range(N_bin):
			...
			if id_cut == False:
				stack_func(img_file, sub_img_file, set_z, set_ra, set_dec, band_str, set_x, set_y, id_cen,
					set_PAs, set_sat_x, set_sat_y, id_half = id_half, sat_PA = sat_PA, sat_ar = sat_ar, 
					rms_file = sub_rms_file, pix_con_file = sub_cont_file )
