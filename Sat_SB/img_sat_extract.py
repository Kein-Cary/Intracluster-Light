...
### === control galaxy extract
def contrl_galx_surround_mask_func(d_file, cat_file, bcg_ra, bcg_dec, bcg_z, ra_set, dec_set, z_set, band, gal_file, out_mask_file, R_cut, 
						offset_file = None, sdss_phot_file = None, extra_cat = None, extra_img = None, 
						alter_fac = None, alt_bright_R = None, alt_G_size = None, stack_info = None, pixel = 0.396):
	"""
	d_file : path where image data saved (include file-name structure:'/xxx/xxx/xxx.xxx')
	cat_file : table of stars and saturated pixels
	bcg_ra, bcg_dec, bcg_z : the cluster or BCG information (for image load, here recording to random points in redMapper)

	----------------
	ra_set, dec_set, z_set : the information of satellites (for location found in frame, and image resampling [z_set] )
	assuming redshift of satellites are the same as cluster
	...
	if stack_info is not None:
		...
		return tmp_ini_x, tmp_ini_y, tmp_cut_x, tmp_cut_y
