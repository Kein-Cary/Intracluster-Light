...
def stack_func(d_file, out_file, z_set, ra_set, dec_set, band, img_x, img_y, id_cen, 
				rms_file = None, pix_con_file = None, id_mean = 0, N_weit = None):
	"""
	...
	N_weit : if weight the cluster image with satellite number or not ('None')
			if N_weit != None, is has the same length as the ra_set array
	"""
	stack_N = len(z_set)
	...
	for jj in range(stack_N):
		...
		if N_weit is None:
			...
			sum_array_A[la0: la1, lb0: lb1][idv] = sum_array_A[la0: la1, lb0: lb1][idv] + N_weit[ jj ] * img_add[idv]
			count_array_A[la0: la1, lb0: lb1][idv] = img_add[idv]

			## tmp array for rms
			pix_f2[la0: la1, lb0: lb1][idv] = pix_f2[la0: la1, lb0: lb1][idv] + N_weit[ jj ] * img_add[idv]**2

			id_nan = np.isnan( count_array_A )
			id_fals = np.where( id_nan == False )
			p_count_A[id_fals] = p_count_A[id_fals] + 1 * N_weit[ jj ]
			count_array_A[la0: la1, lb0: lb1][idv] = np.nan
