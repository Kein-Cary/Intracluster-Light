	...
	for pp in range( 3 ):
		...
		for tt in range( len(R_bins) - 1 ):
			...
			for kk in range( 3 ):
				...
				##.
				out_shufl_file = ( out_path + '%s_clust_frame-lim_Pm-cut_rich_%d-%d_phyR_%d-%dkpc_mem_%s-band_' 
									% (cat_lis[dd], bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str) + 
									'sat-shufl-%d_cat.csv',)[0]

				out_pos_file = ( out_path + '%s_clust_frame-lim_Pm-cut_rich_%d-%d_phyR_%d-%dkpc_mem_%s-band_'
									% (cat_lis[dd], bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str) + 
									'sat-shufl-%d_origin-img_position.csv',)[0]

				out_Ng_file = ( out_path + '%s_clust_frame-lim_Pm-cut_rich_%d-%d_phyR_%d-%dkpc_mem_%s-band_' 
									% (cat_lis[dd], bin_rich[pp], bin_rich[pp + 1], R_bins[tt], R_bins[tt + 1], band_str) + 
