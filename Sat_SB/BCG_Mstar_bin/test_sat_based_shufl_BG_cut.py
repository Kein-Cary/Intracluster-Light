...
out_path = '/home/xkchen/data/SDSS/member_files/Mrich_binned_shufl_img/shufl_cat/'

##. satellite background image with BCGs
# img_file = home + 'photo_files/pos_offset_correct_imgs/mask_img/photo-z_mask_%s_ra%.3f_dec%.3f_z%.3f.fits'
# out_file = ( home + 'member_files/Mrich_binned_shufl_img/mask_img/' + 
# 				'clus_shufl-tract_%s-band_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_img.fits',)[0]

##. satellite background image without BCG
img_file = ( '/home/xkchen/data/SDSS/photo_files/pos_offset_correct_imgs/nobcg_mask_img/' + 
				'photo-z_mask_%s_ra%.3f_dec%.3f_z%.3f.fits',)[0]
out_file = ( home + 'member_files/Mrich_binned_shufl_img/sat_woBCG/mask_img/' + 
				'clus_shufl-tract_%s-band_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_img.fits',)[0]
...
for pp in range( 2 ):
	...
	for tt in range( 3 ):
		...
		for kk in range( 3 ):
			...
			##. shuffle table
			rand_cat = pds.read_csv( out_path + '%s_clust_rich_%d-%d_%s-band_sat-shuffle-%d_position.csv' % 
								(cat_lis[ pp ], bin_rich[tt], bin_rich[tt + 1], band_str, list_order),)
			...
			#. clust cat_file
			clust_cat_file = cat_path + '%s_clust_rich_%d-%d_cat.csv' % (cat_lis[ pp ], bin_rich[tt], bin_rich[tt + 1])
	...
	print('%s, %d-rank, cut Done!' %(cat_lis[pp], rank), )
...
for pp in range( 2 ):
	...
	for tt in range( 3 ):
		...
		for kk in range( 3 ):
			...
			##. satellite table
			dat = pds.read_csv( cat_path + '%s_clust_rich_%d-%d_rgi-common_frame-lim_Pm-cut_exlu-BCG_sat_%s-band_member_pos.csv' 
							% (cat_lis[pp], bin_rich[tt], bin_rich[tt + 1], band_str),)
			...
			pat = pds.read_csv( out_path + '%s_clust_rich_%d-%d_%s-band_sat-shuffle-%d_position.csv' % 
								(cat_lis[ pp ], bin_rich[tt], bin_rich[tt + 1], band_str, list_order),)
			...
			##. satellite background image with BCG
			# d_file = ( home + 'member_files/Mrich_binned_shufl_img/mask_img/' + 
			# 			'clus_shufl-tract_%s-band_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_img.fits',)[0]
			# out_file = ( home + 'member_files/Mrich_binned_shufl_img/resamp_img/' + 
			# 			'clus_shufl-tract_%s-band_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_resamp.fits',)[0]

			##. satellite background image without BCG
			d_file = ( home + 'member_files/Mrich_binned_shufl_img/sat_woBCG/mask_img/' + 
						'clus_shufl-tract_%s-band_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_img.fits',)[0]
			out_file = ( home + 'member_files/Mrich_binned_shufl_img/sat_woBCG/resamp_img/' + 
						'clus_shufl-tract_%s-band_ra%.3f_dec%.3f_z%.3f_sat_ra%.4f_dec%.4f_resamp.fits',)[0]
