    ...
    for dd in range( 2 ):
        ...
        for tt in range( len(R_bins) - 1 ):
            ...
            for kk in range( 3 ):
                ...
                ##.
                sat_sb_file = ( path + '%s_clust_%s_phyR_%d-%dkpc' % (cat_lis[dd], sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                                '_%s-band' % band_str + '_jack-sub-%d_SB-pro_z-ref.h5',)[0]

                bg_sb_file = ( BG_path + '%s_clust_%s_phyR_%d-%dkpc' % (cat_lis[dd], sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
                                '_%s-band_shufl-%d_BG' % (band_str, list_order) + '_Mean_jack_SB-pro_z-ref.h5',)[0]

                out_file = ( out_path + '%s_clust_%s_phyR_%d-%dkpc' % (cat_lis[dd], sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
...
for ll in range( 3 ):
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        for kk in range( 3 ):
            ...
            with h5py.File( path + '%s_clust_%s_phyR_%d-%dkpc' % (cat_lis[qq], sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        for kk in range( 3 ):
            ...
            with h5py.File( BG_path + '%s_clust_%s_phyR_%d-%dkpc' % (cat_lis[qq], sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
    ...
    for tt in range( len(R_bins) - 1 ):
        ...
        for kk in range( 3 ):
            ...
            dat = pds.read_csv( out_path + '%s_clust_%s_phyR_%d-%dkpc' % (cat_lis[qq], sub_name[ ll ], R_bins[tt], R_bins[tt + 1]) + 
    ...
    for kk in range( 3 ):
        ...
        legend_2 = ax1.legend( handles = [l2, l3], 
                    labels = ['Satellite + Background', 'Background' ], loc = 5, frameon = False,)
        ...
        ax1.annotate( s = samp_name[ qq ] + '\n' + line_name[ll] + ', %s-band' % band[kk], xy = (0.25, 0.85), xycoords = 'axes fraction',)

        ax1.set_ylim( y_lim_0[kk][0], y_lim_0[kk][1] )
        ax1.set_ylabel('$\\mu \; [nanomaggy \, / \, arcsec^{2}]$')
        ax1.set_yscale('log')

        plt.savefig('/home/xkchen/%s_clust_%s_sat_%s-band_BG_compare.png' % (cat_lis[qq], sub_name[ll], band[kk]), dpi = 300)
        plt.close()
    ...
    for kk in range( 3 ):
        ...
        ax1.annotate( s = samp_name[qq] + '\n' + line_name[ll] + ', %s-band' % band[kk], xy = (0.65, 0.85), xycoords = 'axes fraction',)
        ...
        sub_ax1.set_ylabel('$\\mu \; / \; \\mu \, (R_{largest})$', labelpad = 8)
        # sub_ax1.set_ylim( daa[0] - 0.30, daa[0] + 0.30 )
        # sub_ax1.set_ylim( 0.5, 1.01 )
        sub_ax1.set_ylim( 0.6, 1.61 )

        sub_ax1.yaxis.set_minor_locator( ticker.AutoMinorLocator() )
        ax1.set_xticklabels( labels = [] )

        plt.savefig('/home/xkchen/%s_clust_%s_sat_%s-band_BG-sub_compare.png' % (cat_lis[qq], sub_name[ll], band[kk]), dpi = 300)
        plt.close()
