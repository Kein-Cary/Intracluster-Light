...
for pp in range( rank, rank + 1 ):
	...
	for dd in range( 3 ):
		...
		for tt in range( 3 ):
			...
			##. set shuffle time is 20
			for kk in range( 20 ):

				##. cluster catalog
				cat = pds.read_csv( cat_path + '%s_clust_rich_%d-%d_cat.csv' % (cat_lis[ pp ], bin_rich[tt], bin_rich[tt + 1]),)
				...
				out_data.to_csv( out_path + '%s_clust_rich_%d-%d_%s-band_sat-shuffle-%d_position.csv' % 
								(cat_lis[ pp ], bin_rich[tt], bin_rich[tt + 1], band_str, kk),)
	...
	for kk in range( 3 ):
		...
		for tt in range( 3 ):
			...
			for dd in range( N_shufl ):
				...
				##.
				dat = pds.read_csv( out_path + '%s_clust_rich_%d-%d_%s-band_sat-shuffle-%d_position.csv' % 
								(cat_lis[ pp ], bin_rich[kk], bin_rich[kk + 1], band_str, dd),)
				...
				##. entire all sample
				dat = pds.read_csv( cat_path + '%s_clust_rich_%d-%d_cat.csv' % (cat_lis[ pp ], bin_rich[kk], bin_rich[kk + 1]),)
				...
				fill = dict( zip( keys, values ) )
				out_data = pds.DataFrame( fill )
				out_data.to_csv( out_path + '%s_clust_rich_%d-%d_%s-band_sat-shuffle-%d_shufl-sat-Ng.csv' % 
								(cat_lis[ pp ], bin_rich[kk], bin_rich[kk + 1], band_str, dd),)
