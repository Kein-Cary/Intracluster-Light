...
low_M_r, low_M_sb, low_M_err = [], [], []
for ii in range( 3 ):

	with h5py.File( path + 'photo-z_match_gri-common_low_BCG_star-Mass_%s-band_Mean_jack_SB-pro_z-ref.h5' % band[ii], 'r') as f:
		tt_r = np.array(f['r'])
		tt_sb = np.array(f['sb'])
		tt_err = np.array(f['sb_err'])

	low_M_r.append( tt_r )
	low_M_sb.append( tt_sb )
	low_M_err.append( tt_err )

hi_M_r, hi_M_sb, hi_M_err = [], [], []
for ii in range( 3 ):

	with h5py.File( path + 'photo-z_match_gri-common_high_BCG_star-Mass_%s-band_Mean_jack_SB-pro_z-ref.h5' % band[ii], 'r') as f:
		tt_r = np.array(f['r'])
		tt_sb = np.array(f['sb'])
		tt_err = np.array(f['sb_err'])
	hi_M_r.append( tt_r )
	hi_M_sb.append( tt_sb )
	hi_M_err.append( tt_err )
...
for formz_dex in (0, 1):
	...
	for kk in range( 3 ):
		...
		if formz_dex == 0:
			tt_r, tt_sb, tt_err = low_M_r[kk], low_M_sb[kk], low_M_err[kk]
		else:
			tt_r, tt_sb, tt_err = hi_M_r[kk], hi_M_sb[kk], hi_M_err[kk]
...
plt.savefig('/home/xkchen/figs/Mass-bin_sample_BG-sub-SB_compare.png', dpi = 300)
plt.close()
...
plt.savefig('/home/xkchen/figs/rgi-band_Mass_sample_BG-sub-SB_compare.png', dpi = 300)
plt.close()
...
plt.savefig('/home/xkchen/figs/Mass_color_compare.png', dpi = 300)
plt.close()
...
# color
pre_hi_c = pds.read_csv( pre_path + 'high_BCG_star-Mass_color_profile.csv' )
...
pre_low_c = pds.read_csv( pre_path + 'low_BCG_star-Mass_color_profile.csv' )
...
plt.subplots_adjust( hspace = 0.)
plt.savefig('/home/xkchen/figs/Mass_color_check.png', dpi = 300)
plt.close()
