...
for ll in range( len( cen_lim_R ) ):
	...
	if id_fixn == True:
		out_data.to_csv( fit_path + 
					'%stotal_%s-band-based_xi2-sigma_mid-region_Lognorm_R-%dkpc_lim.csv' % (dered_str, band_str, cen_lim_R[ll]),)

	if id_fixn == False:
		out_data.to_csv( fit_path + 
					'%stotal_%s-band-based_xi2-sigma_mid-region_Lognorm_R-%dkpc_lim_free-n.csv' % (dered_str, band_str, cen_lim_R[ll]),)
...
def fixn_compare():
	...
	f_Ie, f_Re, f_ne = np.array( p_dat['Ie'] )[0], np.array( p_dat['Re'] )[0], np.array( p_dat['ne'] )[0]

	mod_cen_M_2Mpc = sersic_func( 2e3, 10**f_Ie, f_Re, f_ne)
	mod_cen_M = sersic_func( new_R, 10**f_Ie, f_Re, f_ne) - mod_cen_M_2Mpc

	fixn_cen_params.append( [f_Ie, f_Re, f_ne] )
	fixn_cen_SM_arr.append( mod_cen_M )


	mid_pat = pds.read_csv( fit_path + '%stotal_gri-band-based_xi2-sigma_mid-region_Lognorm-mcmc-fit.csv' % dered_str,)
	lg_SM_fit, Rt_fit, sigm_tt_fit = np.array( mid_pat['lg_M0'] )[0], np.array( mid_pat['R_t'] )[0], np.array( mid_pat['sigma_t'] )[0]

	mod_trans_M = log_norm_func( new_R, lg_SM_fit, Rt_fit, sigm_tt_fit ) - log_norm_func( 2e3, lg_SM_fit, Rt_fit, sigm_tt_fit )

	fixn_mid_params.append( [lg_SM_fit, Rt_fit, sigm_tt_fit] )
	fixn_mid_SM_arr.append( mod_trans_M )


	devi_M = surf_M - ( xi_to_Mf( obs_R) - sigma_2Mpc ) * 10**lg_fb_gi - ( sersic_func( obs_R, 10**f_Ie, f_Re, f_ne) - mod_cen_M_2Mpc )
	devi_err = surf_M_err
	...
	Ie_30n4, Re_30n4, ne_30n4 = np.array( p_dat['Ie'] )[0], np.array( p_dat['Re'] )[0], np.array( p_dat['ne'] )[0]

	cen_M_2Mpc_30n4 = sersic_func( 2e3, 10**Ie_30n4, Re_30n4, ne_30n4)
	cen_M_30n4 = sersic_func( new_R, 10**Ie_30n4, Re_30n4, ne_30n4) - cen_M_2Mpc_30n4

	fixn_cen_params.append( [Ie_30n4, Re_30n4, ne_30n4] )
	fixn_cen_SM_arr.append( cen_M_30n4 )


	mid_pat = pds.read_csv( fit_path + 
						'%stotal_gri-band-based_xi2-sigma_mid-region_Lognorm_R-30kpc_lim.csv' % dered_str,)
	SM_fit_30n4, Rt_fit_30n4, sigm_fit_30n4 = np.array( mid_pat['lg_M0'] )[0], np.array( mid_pat['R_t'] )[0], np.array( mid_pat['sigma_t'] )[0]

	trans_M_30n4 = log_norm_func( new_R, SM_fit_30n4, Rt_fit_30n4, sigm_fit_30n4 ) - log_norm_func( 2e3, SM_fit_30n4, Rt_fit_30n4, sigm_fit_30n4 )

	fixn_mid_params.append( [SM_fit_30n4, Rt_fit_30n4, sigm_fit_30n4] )
	fixn_mid_SM_arr.append( trans_M_30n4 )


	devi_M = surf_M - ( xi_to_Mf( obs_R) - sigma_2Mpc ) * 10**lg_fb_gi - (sersic_func( obs_R, 10**Ie_30n4, Re_30n4, ne_30n4) - cen_M_2Mpc_30n4 )
	devi_err = surf_M_err
	...
	devi_R_30n4 = obs_R[ id_lim == False]
	devi_M_30n4 = devi_M[ id_lim == False ]
	devi_err_30n4 = devi_err[ id_lim == False ]

	fixn_diffi_SM_arr.append( [ devi_R_30n4, devi_M_30n4, devi_err_30n4 ] )
...
for ll in range( 2 ):
	...
	mid_pat = pds.read_csv( fit_path + 
					'%stotal_gri-band-based_xi2-sigma_mid-region_Lognorm_R-%dkpc_lim_free-n.csv' % (dered_str, cen_lim_R[ll]),)
	SM_fit_kk, Rt_fit_kk, sigm_fit_kk = np.array( mid_pat['lg_M0'] )[0], np.array( mid_pat['R_t'] )[0], np.array( mid_pat['sigma_t'] )[0]

	mod_trans_M = log_norm_func( new_R, SM_fit_kk, Rt_fit_kk, sigm_fit_kk ) - log_norm_func( 2e3, SM_fit_kk, Rt_fit_kk, sigm_fit_kk )

	freen_mid_params.append( [SM_fit_kk, Rt_fit_kk, sigm_fit_kk] )
	freen_mid_SM_arr.append( mod_trans_M )
...
sub_ax1.axhline( y = 0.25, color = 'g', ls = ':')
sub_ax1.axhline( y = 0.225, color = 'r', ls = ':')
sub_ax1.axhline( y = 0.15, color = 'b', ls = ':')
