...
"""
### === sample match and check overall properties
...
for mm in range( 2 ):
	...
	for kk in range( 3 ):
		...
		if file_s == 'BCG_Mstar_bin':
			cc_dat = pds.read_csv( '/home/xkchen/mywork/ICL/data/BCG_stellar_mass_cat/photo_z_gri_common/' + 
								'%s_%s-band_photo-z-match_rgi-common_BCG-pos_cat.csv' % (cat_lis[mm], band[kk]),)

			dat = pds.read_csv( '/home/xkchen/mywork/ICL/data/BCG_stellar_mass_cat/photo_z_match/sample_view/' + 
								'%s_%s-band_photo-z-match_BCG-pos_cat.csv' % (cat_lis[mm], band[kk]),)

		
		if file_s == 'rich_bin_fixed_BCG_M':
			cc_dat = pds.read_csv( '/home/xkchen/mywork/ICL/data/cat_z_form/bcg_M_based_cat/rich_bin/' + 
								'%s_%s-band_photo-z-match_rgi-common_BCG-pos_cat.csv' % (cat_lis[mm], band[kk]),)

			dat = pds.read_csv( '/home/xkchen/mywork/ICL/data/cat_z_form/bcg_M_based_cat/rich_bin/' + 
								'%s_%s-band_photo-z-match_BCG-pos_cat.csv' % (cat_lis[mm], band[kk]),)


		if file_s == 'age_bin_fixed_BCG_M':
			cc_dat = pds.read_csv( '/home/xkchen/mywork/ICL/data/cat_z_form/bcg_M_based_cat/age_bin/' + 
								'%s_%s-band_photo-z-match_rgi-common_BCG-pos_cat.csv' % (cat_lis[mm], band[kk]),)
		...
		match_rich = lim_rich[ sub_index ]

		cc_index = simple_match( out_ra, out_dec, out_z, cc_ra, cc_dec, cc_z )
		cc_match_rich = lim_rich[ sub_index ]


		plt.figure()
		plt.hist( z, bins = 45, density = True, color = 'b', histtype = 'step', label = 'matched', alpha = 0.5,)
		plt.axvline( x = np.mean(z), ls = '--', color = 'b', alpha = 0.5, label = 'mean')
		plt.axvline( x = np.median(z), ls = '-', color = 'b', alpha = 0.5, label = 'median')

		plt.hist( cc_z, bins = 45, density = True, color = 'r', histtype = 'step', label = 'gri-common', alpha = 0.5)
		plt.axvline( x = np.mean(cc_z), ls = '--', color = 'r', alpha = 0.5,)
		plt.axvline( x = np.median(cc_z), ls = '-', color = 'r', alpha = 0.5,)
		plt.legend( loc = 2 )
		plt.xlabel('$z_{photo}$')
		plt.savefig('/home/xkchen/%s_%s-band_sample_z_compare.png' % (cat_lis[mm], band[kk]), dpi = 300)
		plt.close()


		plt.figure()
		plt.hist( match_rich, bins = 45, density = True, color = 'b', histtype = 'step', label = 'matched', alpha = 0.5,)
		plt.axvline( x = np.mean(match_rich), ls = '--', color = 'b', alpha = 0.5, label = 'mean')
		plt.axvline( x = np.median(match_rich), ls = '-', color = 'b', alpha = 0.5, label = 'median')

		plt.hist( cc_match_rich, bins = 45, density = True, color = 'r', histtype = 'step', label = 'gri-common', alpha = 0.5,)
		plt.axvline( x = np.mean(cc_match_rich), ls = '--', color = 'r', alpha = 0.5,)
		plt.axvline( x = np.median(cc_match_rich), ls = '-', color = 'r', alpha = 0.5,)
		plt.legend( loc = 1 )
		plt.xscale('log')
		plt.xlabel('$\\lambda$')
		plt.savefig('/home/xkchen/%s_%s-band_sample_rich_compare.png' % (cat_lis[mm], band[kk]), dpi = 300)
		plt.close()
...
for kk in range( Ns ):
	...
	try:
		...
		cMod_mag_g.append( np.array( cat['cModelMag_g'] )[0] )
		cMod_mag_r.append( np.array( cat['cModelMag_r'] )[0] )
...
cMod_mag_r = np.array( cMod_mag_r )
cMod_mag_g = np.array( cMod_mag_g )


keys = [ 'ra', 'dec', 'z', 'mod_r', 'mod_g', 'mod_i', 'g-r', 'g-i', 'r-i', 'cMod_mag_i', 'cMod_mag_r', 'cMod_mag_g', 'objID']
values = [ ext_ra, ext_dec, ext_z, tmp_r_mag, tmp_g_mag, tmp_i_mag, tmp_gr, tmp_gi, tmp_ri, cMod_mag_i, cMod_mag_r, cMod_mag_g, ext_ID ]
fill = dict(zip( keys, values) )
out_data = pds.DataFrame( fill )
