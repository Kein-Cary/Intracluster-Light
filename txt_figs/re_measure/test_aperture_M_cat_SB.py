...
def sub_SM_ratio():
	...
	lo_eta_R, lo_eta, lo_eta_err = np.array(lo_eat_dat['R']), np.array(lo_eat_dat['medi_ref_M/M_tot']), np.array(lo_eat_dat['medi_ref_M/M_tot-err'])

	hi_eat_dat = pds.read_csv( out_path + '%s_gri-band_corrected-aveg-M-ratio_to_total-sample%s.csv' % (cat_lis[1], dered_str),)
	hi_eta_R, hi_eta, hi_eta_err = np.array(hi_eat_dat['R']), np.array(hi_eat_dat['medi_ref_M/M_tot']), np.array(hi_eat_dat['medi_ref_M/M_tot-err'])
