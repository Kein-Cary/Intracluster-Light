...
BG_path = '/home/xkchen/figs/fixz_age_bin_test/BGs/'
path = '/home/xkchen/figs/fixz_age_bin_test/SBs/'
cat_path = '/home/xkchen/figs/fixz_age_bin_test/'
...
for mm in range( 1,2 ):
	...
	for kk in range( 3 ):
		...
		with h5py.File( path + 'photo-z_match_gri-common_%s_%s-band_Mean_jack_SB-pro_z-ref_pk-off.h5' % (cat_lis[mm], band[kk]), 'r') as f:
			tt_r = np.array(f['r'])
			tt_sb = np.array(f['sb'])
...
for mm in range( 2 ):
	...
	E_dat = pds.read_csv( cat_path + '%s_BCG_dust_value.csv' % cat_lis[mm],)
...
def surf_M_estimate():
	...
	out_path = '/home/xkchen/figs/fixz_age_bin_test/SM_pro/'
	...
	for mm in range( 2 ):
		...
		fit_surfM_func( N_bin, band_str, low_R_lim, up_R_lim, sub_sb_file, sub_sm_file, Dl_ref, z_ref,
	...
	for mm in range( 2 ):
		...
		aveg_R_0, aveg_surf_m, aveg_surf_m_err = arr_jack_func( tmp_SM, tmp_r, N_bin )[:3]
		aveg_R_1, aveg_lumi, aveg_lumi_err = arr_jack_func( tmp_Li, tmp_r, N_bin )[:3]
		aveg_R_2, aveg_lgM, aveg_lgM_err = arr_jack_func( tmp_lgSM, tmp_r, N_bin )[:3]
	...
	#. surface mass profile correction (fixed aperture size)
	...
	# fix_R_dat = pds.read_csv( '/home/xkchen/figs/re_measure_SBs/SM_profile/' + 
	# 							'total-sample_%s-band_based_BCGM-match_R.csv' % band_str )
	...
	for mm in range( 2 ):
		...
		#...sample properties
		p_dat = pds.read_csv( '/home/xkchen/figs/fixz_age_bin_test/' + 
							'%s_photo-z-match_rgi-common-cat_params.csv' % cat_lis[mm] )
	...
	calib_cat = pds.read_csv( out_path + '%s_gri-band-based_M_calib-f_with-dered.csv' % file_s )
	...
	#. ratio of mass profile without fixed-R mass correction
	for mm in range( 2 ):
	...
	dat = pds.read_csv( out_path + '%s_gri-band-based_corrected_aveg-jack_mass-Lumi_with-dered.csv' % cat_lis[0],)
	...
	dat = pds.read_csv( out_path + '%s_gri-band-based_corrected_aveg-jack_mass-Lumi_with-dered.csv' % cat_lis[1],)
	...
	lo_eat_dat = pds.read_csv( out_path + '%s_gri-band_corrected-aveg-M-ratio_to_total-sample_with-dered.csv' % cat_lis[0],)
	# lo_eat_dat = pds.read_csv( out_path + '%s_gri-band_corrected-aveg-M-ratio_to_total-sample.csv' % cat_lis[0],)
	lo_eta_R, lo_eta, lo_eta_err = np.array(lo_eat_dat['R']), np.array(lo_eat_dat['M/M_tot']), np.array(lo_eat_dat['M/M_tot-err'])

	hi_eat_dat = pds.read_csv( out_path + '%s_gri-band_corrected-aveg-M-ratio_to_total-sample_with-dered.csv' % cat_lis[1],)
	# hi_eat_dat = pds.read_csv( out_path + '%s_gri-band_corrected-aveg-M-ratio_to_total-sample.csv' % cat_lis[1],)
...
mu_dat = pds.read_csv( BG_path + '%s_dered_color_profile.csv' % cat_lis[1] )
hi_c_r_1, hi_dered_gr, hi_dered_gr_err = np.array( mu_dat['R_kpc'] ), np.array( mu_dat['g-r'] ), np.array( mu_dat['g-r_err'] )
hi_dered_gr = signal.savgol_filter( hi_dered_gr, 7, 3)

mu_dat = pds.read_csv( BG_path + '%s_dered_color_profile.csv' % cat_lis[0] )
lo_c_r_1, lo_dered_gr, lo_dered_gr_err = np.array( mu_dat['R_kpc'] ), np.array( mu_dat['g-r'] ), np.array( mu_dat['g-r_err'] )
lo_dered_gr = signal.savgol_filter( lo_dered_gr, 7, 3)
...
ax.plot( hi_c_r_1, hi_dered_gr, ls = '--', color = 'r', alpha = 0.75, linewidth = 1, label = fig_name[1] + ', deredden')
ax.fill_between( hi_c_r_1, y1 = hi_dered_gr - hi_dered_gr_err, y2 = hi_dered_gr + hi_dered_gr_err, color = 'r', alpha = 0.15,)

ax.plot( lo_c_r_1, lo_dered_gr, ls = '--', color = 'b', alpha = 0.75, linewidth = 1, label = fig_name[0] + ', deredden')
ax.fill_between( lo_c_r_1, y1 = lo_dered_gr - lo_dered_gr_err, y2 = lo_dered_gr + lo_dered_gr_err, color = 'b', alpha = 0.15,)
