...
def norm_err_fit_f(p, x, y, yerr):

	Am0, Rt0, sigm_t0 = p[:]

	_mpdf = norm_func( x, Am0, Rt0, sigm_t0 )
...
for mm in range( 2 ):
	...
	E_return = optimize.minimize( norm_err_fit_f, x0 = np.array( po ), args = (mid_R, mid_M_eta, mid_M_eta_err), 
	...
	#...
	fit_cross_0 = norm_func( obs_R, Am_fit, Rt_fit, sigm_tt_fit)
	fit_mid_SM_0 = fit_cross_0 * ( fit_sum ) / ( 1 - fit_cross_0 )

	chi_SM0 = norm_func( mid_R, Am_fit, Rt_fit, sigm_tt_fit ) * tmp_tot_M_f( mid_R )
	delta = chi_SM0 - mid_M
	...
	keys = ['Am', 'Rt', 'sigma_t', 'chi2nv']
	values = [ Am_fit, Rt_fit, sigm_tt_fit, chi2nv_0 ]
	fill = dict( zip( keys, values) )
	out_data = pds.DataFrame( fill, index = ['k', 'v'])
	out_data.to_csv( fit_path + '%s_%s-band-based_mid-region_ratio-based_norm-fit%s.csv' % (cat_lis[mm], band_str, dered_str),)


	#. SM(r) ratio with log-normal fitting
	mid_dat = pds.read_csv( fit_path + '%s_%s-band-based_mid-region_Lognorm_ratio-based_fit%s.csv' % (cat_lis[mm], band_str, dered_str),)
	cp_Am_fit, cp_Rt_fit, cp_sigm_tt_fit = np.array( mid_dat['Am'])[0], np.array( mid_dat['Rt'])[0], np.array( mid_dat['sigma_t'])[0]
	chi2nv_1 = np.array( mid_dat['chi2nv'] )[0]

	fit_cross_1 = log_norm_func( obs_R, cp_Am_fit, cp_Rt_fit, cp_sigm_tt_fit)
	fit_mid_SM_1 = fit_cross_1 * ( fit_sum ) / ( 1 - fit_cross_1 )
	...
	ax1.plot( obs_R, fit_mid_SM_0, ls = '-', color = 'b', alpha = 0.75, label = 'normal fit',)
	ax1.plot( obs_R, fit_mid_SM_1, ls = ':', color = 'r', alpha = 0.75, label = 'log-normal fit',)
	...
	sub_ax1.errorbar( mid_R, mid_M_eta, yerr = mid_M_eta_err, ls = 'none', marker = 's', ms = 8, mec = 'k', 
				mfc = 'none', ecolor = 'k', alpha = 0.75, capsize = 3,)
	sub_ax1.plot( obs_R, fit_cross_0, 'b-', alpha = 0.75,)
	sub_ax1.plot( obs_R, fit_cross_1, 'r--', alpha = 0.75,)
...
fill = dict( zip( keys, values) )
out_data = pds.DataFrame( fill, index = ['k', 'v'])
out_data.to_csv( fit_path + '%stotal_%s-band-based_mid-region_ratio-based_norm-fit.csv' % (dered_str, band_str),)


#. SM(r) ratio with log-Normal fit
mid_dat = pds.read_csv( fit_path + '%stotal_%s-band-based_mid-region_Lognorm_ratio-based_fit.csv' % (dered_str, band_str),)
cp_Am_fit, cp_Rt_fit, cp_sigm_tt_fit = np.array( mid_dat['Am'])[0], np.array( mid_dat['Rt'])[0], np.array( mid_dat['sigma_t'])[0]
chi2nv_1 = np.array( mid_dat['chi2nv'] )[0]
