...
# rich_divid_set = '/home/xkchen/tmp_run/data_files/figs/rich-bin_fixed-BCG-M_divid_line_params.csv'

##. extended + LOWZ sample
cat = pds.read_csv('/home/xkchen/mywork/ICL/data/BCG_Mstar_extend_cat/' + 'Extend_BCGM_bin_cat.csv')
ra, dec, z = np.array(cat['ra']), np.array(cat['dec']), np.array(cat['z'])
rich, z_form = np.array(cat['rich']), np.array(cat['z_form'])
lg_Mstar = np.array( cat['lg_Mbcg'] ) # Mass unit : M_sun / h^2

# rich_divid_set = '/home/xkchen/mywork/ICL/data/BCG_Mstar_extend_cat/rich_bin_fixed_bcgM/rich-bin_fixed-BCG-M_divid_line_params.csv'
rich_divid_set = '/home/xkchen/mywork/ICL/data/BCG_Mstar_extend_cat/re_bin_rich_bin/rich-bin_fixed-BCG-M_divid_line_params.csv'
...
for pp in range( 2 ):
	...
	dat = pds.read_csv( '/home/xkchen/figs/extend_bcgM_cat/SM_pros/' + 'photo-z_tot-BCG-star-Mass_%s-band-based_aveg-jack_mass-Lumi_with-dered.csv' % band_str,)
	...
	if file_s == 'BCG_Mstar_bin':
		...
		for ii in range( pre_N - 1):
			...
			std_lgM.append( np.std( lim_lgM - 2 * np.log10( h ) ) / np.sqrt( len(lim_lgM) ) )
		...
		div_dat = pds.read_csv( rich_divid_set )
		pa_x2, pb_x, p_c0 = np.array( div_dat['a_x^2'] )[0], np.array( div_dat['b_x'] )[0], np.array( div_dat['c'] )[0]

		cp_lgMx = np.linspace(9, 12, 100)  # lgM_x is mass in unit M_sun / h^2
		cp_line = pa_x2 * (cp_lgMx)**2 + pb_x * cp_lgMx + p_c0
		cp_lgMx = cp_lgMx - 2 * np.log10( h )
		...
		ax0.plot( _point_rich, line_divi, ls = '-', color = 'k', alpha = 0.75, linewidth = 2, label = 
				'$ 0.45 \, {\\rm \\mathcal{lg} } \, \\lambda $' + '$ \; + \; %.2f$' % (10.518 - 2 * np.log10(h) ),)
		...
		ax0.legend( loc = 3, frameon = False, fontsize = 20, markerfirst = True, handletextpad = 0.3,)
	...
	if file_s == 'rich_bin_fixed_BCG_M':
		...
		##... extended sample
		pre_N = 7
		bins_edg = np.linspace( 10.25, 11.75, pre_N) #11.75
		...
		for ii in range( len(bins_edg) - 1 ):
			...
			# std_rich.append( np.std( lim_rich ) )
			std_rich.append( np.std( lim_rich ) / np.sqrt( len( lim_rich ) ) )
		...
		ax0.errorbar( cp_cen_lgM[1:-1], medi_rich[1:-1], yerr = std_rich[1:-1], xerr = None, color = 'k', marker = 'o', ls = 'none', ecolor = 'k', 
			mec = 'k', mfc = 'k', capsize = 2, markersize = 7,)
		...
		ax0.plot( cp_lgMx, 10**cp_line, ls = '-', color = 'k', alpha = 0.75, linewidth = 2, 
			label = 
			'$ %.2f \, {\\rm \\mathcal{lg} }^{2} \, M^{\\mathrm{BCG}}_{\\ast} \, %.2f \, {\\rm \\mathcal{lg} } \, M^{\\mathrm{BCG}}_{\\ast} \; + \; %.2f$' 
			% ( Pf[2], Pf[1], Pf[0] + p_fh),)


		# ax0.plot( tt_sep_line, _point_rich, color = 'k', ls = '--', alpha = 0.75, linewidth = 2,)

		ax0.legend( loc = 2, frameon = True, fontsize = 20, facecolor = 'w', edgecolor = 'w', framealpha = 0.8, 
					borderaxespad = 0.5, borderpad = 0.1, handletextpad = 0.3,)

		ax0.text( 11.9, 42, s = 'High $ \\lambda $', fontsize = 24, color = 'k', rotation = 40,)
		ax0.text( 11.9, 28, s = 'Low $ \\lambda $', fontsize = 24, color = 'k', rotation = 40,)
