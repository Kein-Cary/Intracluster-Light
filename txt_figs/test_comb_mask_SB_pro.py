...
for kk in range( 3 ):
	...
	with h5py.File( 
		path + 'photo-z_match_gri-common_%s_%s-band_Mean_jack_SB-pro_z-ref_pk-off.h5' % (cat_lis[0], band[kk]), 'r') as f:
		tt_r = np.array(f['r'])
		tt_sb = np.array(f['sb'])
		tt_err = np.array(f['sb_err'])
...
for kk in range( 3 ):
	...
	with h5py.File( 
		path + 'photo-z_match_gri-common_%s_%s-band_Mean_jack_SB-pro_z-ref_pk-off.h5' % (cat_lis[1], band[kk]), 'r') as f:
		tt_r = np.array(f['r'])
		tt_sb = np.array(f['sb'])
		tt_err = np.array(f['sb_err'])
...
for kk in range( 3 ):
	...
	with h5py.File( 
		pre_path + 'photo-z_match_gri-common_%s_%s-band_Mean_jack_SB-pro_z-ref_pk-off.h5' % (cat_lis[0], band[kk]), 'r') as f:
		tt_r = np.array(f['r'])
		tt_sb = np.array(f['sb'])
		tt_err = np.array(f['sb_err'])
...
for kk in range( 3 ):
	...
	with h5py.File( 
		pre_path + 'photo-z_match_gri-common_%s_%s-band_Mean_jack_SB-pro_z-ref_pk-off.h5' % (cat_lis[1], band[kk]), 'r') as f:
		tt_r = np.array(f['r'])
		tt_sb = np.array(f['sb'])
		tt_err = np.array(f['sb_err'])
...
for kk in range( 3 ):
	...
	ax.plot( low_r[kk], low_sb[kk], ls = m_ls[kk], color = 'b', alpha = 0.75,)
	ax.fill_between( low_r[kk], y1 = low_sb[kk] - low_err[kk], 
		y2 = low_sb[kk] + low_err[kk], color = 'b', alpha = 0.12,)

	ax.plot( hi_r[kk], hi_sb[kk], ls = m_ls[kk], color = 'r', alpha = 0.75, label = '%s band' % band[kk])
	ax.fill_between( hi_r[kk], y1 = hi_sb[kk] - hi_err[kk], 
		y2 = hi_sb[kk] + hi_err[kk], color = 'r', alpha = 0.12,)

	ax.plot( pre_low_r[kk], pre_low_sb[kk], ls = m_ls[kk], color = 'k', alpha = 0.45,)
	ax.plot( pre_hi_r[kk], pre_hi_sb[kk], ls = m_ls[kk], color = 'k', alpha = 0.45,)

legend_2 = plt.legend( [ fig_name[0], fig_name[1], 'uncorrected flux scaling'], 
	loc = 3, frameon = False, fontsize = 15,)
...
plt.savefig('/home/xkchen/%s_sample_SB_compare.png' % file_s, dpi = 300)
plt.close()
