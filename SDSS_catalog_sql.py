...
def put_sql(z_set, ra_set, dec_set, ref_ra, ref_dec, out_file,):
	...
	url_link = "http://skyserver.sdss.org/dr12/en/tools/search/x_sql.aspx"
	...
	doc = open('/home/xkchen/%d_rank_record.txt' % rank, 'w')
	...
	for q in range(Nz):
		...
		sdss_sql(url_link, data_set, fmt, print_file, id_print = False,)
...
out_file = home + 'new_sql_star_cat/random/source_SQL_Z%.3f_ra%.3f_dec%.3f.txt'
...
#print('%d rank random finished !')
'''

# dat = pds.read_csv(home + 'selection/SDSS_spec-sample_sql_cat.csv')
dat = pds.read_csv( '/home/xkchen/mywork/ICL/data/cat_select/SDSS_spec-sample_sql_cat.csv' )
ra, dec, z = np.array(dat['ra']), np.array(dat['dec']), np.array(dat['z'])
ref_ra, ref_dec = np.array(dat['ref_ra']), np.array(dat['ref_dec'])
...
# out_file = home + 'new_sql_star_cat/dr12/source_SQL_Z%.3f_ra%.3f_dec%.3f.txt'
out_file = '/home/xkchen/source_SQL_Z%.3f_ra%.3f_dec%.3f.txt'

#sdss_sql_star(z[N_sub0:N_sub1], ra[N_sub0:N_sub1], dec[N_sub0:N_sub1], ref_ra[N_sub0:N_sub1], ref_dec[N_sub0:N_sub1], out_file,)
# put_sql(z[N_sub0:N_sub1], ra[N_sub0:N_sub1], dec[N_sub0:N_sub1], ref_ra[N_sub0:N_sub1], ref_dec[N_sub0:N_sub1], out_file,)

put_sql(z[:2], ra[:2], dec[:2], ref_ra[:2], ref_dec[:2], out_file,)
