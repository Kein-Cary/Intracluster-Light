...
'''
##### stars query
def sdss_query(z_set, ra_set, dec_set):
	import h5py
	import numpy as np
	import astropy.io.fits as fits
	...
	r_select = 0.558 ## 2 * diagonal line length
	...
	for q in range(Nz):
		...
		doc = open(load + 'source_SQL_Z%.3f_ra%.3f_dec%.3f.txt'%(time, ra, dec), 'w')
		print(s, file = doc)
		doc.close()

	return

set_ra = np.array([359.75253625, 191.0523974, 17.2688937, 197.56986305, 130.3783955])
set_dec = np.array([28.05588246, 16.85510289, 21.10405202, 19.96462519, 21.40908432])
set_z = np.array([0.23799674, 0.26528448, 0.25037688, 0.25795066, 0.24528672])
sdss_query(set_z, set_ra, set_dec)
'''
