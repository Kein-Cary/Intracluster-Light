...
for kk in range( len(k_r) ):
	...
	sub_x2 = x[ipx]
	sub_y2 = y[ipx]
	sub_A2 = lr_iso[ipx] * tau
	sub_B2 = sr_iso[ipx] * tau
	...
	hdu.data = mask_img
	hdu.header = head
	hdu.writeto(load + 'star_class/mask_ra%.3f_dec%.3f_z%.3f_%.1fR-fwhm_stars_satu.fits' % (ra_g, dec_g, z_g, k_r[kk] / 2), overwrite = True)
	...
	with h5py.File(load + 'star_class/sub_mean_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
	...
	plt.close()
	'''
	with h5py.File(load + 'star_class/sub_patch_aveg_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
		f['aveg'] = np.array(aveg)

	with h5py.File(load + 'star_class/sub_patch_sigma_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
		f['sigma'] = np.array(sigma)

	with h5py.File(load + 'star_class/tot_flux_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
...
for kk in range( len(k_r) ):
	...
	with h5py.File(load + 'star_class/sub_patch_aveg_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
	...
	with h5py.File(load + 'star_class/sub_patch_sigma_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
	...
	with h5py.File(load + 'star_class/tot_flux_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
...
for kk in range( len(k_r) ):
	...
	with h5py.File(load + 'star_class/sub_mean_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_satu.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
