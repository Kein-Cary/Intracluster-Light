...
for kk in range( len(k_r) ):
	...
	Kron = 16 ## twice of kron factor, since A, B are semi-axis
	a = Kron * A
	b = Kron * B
	...
	sub_x0 = x[ic]
	sub_y0 = y[ic]
	sub_A0 = lr_iso[ic] * tau
	sub_B0 = sr_iso[ic] * tau
	sub_chi0 = set_chi[ic]
	...
	sub_x2 = x[ipx]
	sub_y2 = y[ipx]
	sub_A2 = lr_iso[ipx] * 30
	sub_B2 = sr_iso[ipx] * 30
	...
	hdu.data = mask_img
	hdu.header = head
	hdu.writeto(load + 'star_class/mask_ra%.3f_dec%.3f_z%.3f_%.1fR-fwhm_stars.fits' % (ra_g, dec_g, z_g, k_r[kk] / 2), overwrite = True)

	plt.figure()
	ax = plt.subplot(111)
	ax.set_title('$ source \; masking \; [%.1f (FWHM / 2)] $' % (k_r[kk] / 2) )
	...
	with h5py.File(load + 'star_class/sub_mean_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
	...
	plt.close()
	'''
	with h5py.File(load + 'star_class/sub_patch_aveg_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_norm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
		f['aveg'] = np.array(aveg)

	with h5py.File(load + 'star_class/sub_patch_sigma_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_norm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
		f['sigma'] = np.array(sigma)

	with h5py.File(load + 'star_class/tot_flux_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_norm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'w') as f:
...
for kk in range( len(k_r) ):
	...
	with h5py.File(load + 'star_class/sub_patch_aveg_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_norm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
	...
	with h5py.File(load + 'star_class/sub_patch_sigma_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_norm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
	...
	with h5py.File(load + 'star_class/tot_flux_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm_norm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
...
ax.axhline(y = aveg_sb[-1], ls = '--', color = 'g', alpha = 0.5)
...
for kk in range( len(k_r) ):
	...
	with h5py.File(load + 'star_class/sub_mean_ra%.3f_dec%.3f_z%.3f_8.0R-kron_%.1fR-fwhm.h5' % (ra_g, dec_g, z_g, k_r[kk] / 2), 'r') as f:
