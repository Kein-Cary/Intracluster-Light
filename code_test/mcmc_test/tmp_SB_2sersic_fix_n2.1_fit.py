...
def prior_p_func( p ):
	...
	mu0, re_0, ne_0, mu1, re_1 = p[:]
	if ( 0 < mu0 < 5e0 ) and ( 5 < re_0 < 1e2) and ( 1 < ne_0 < 10 ) and (0 < mu1 < 1e-2) and (2e2 < re_1 < 3e3):
		return 0.0
	return -np.inf
...
def prior_p_fix1_func( p ):
	...
	if ( 0 < mu0 < 5e0 ) * ( 5 < re_0 < 1e2) * ( 1 < ne_0 < 10 ):
		return 0
	return -np.inf
...
for tt in range( 3 ):
	...
	## array saving
	# file_name = out_path + 'low_BCG_star-Mass_%s-band_2-sersic_out=2.1_fit_mcmc_arr.h5' % band[tt]
	...
	( Ie1_arr, Re1_arr, ne1_arr, Ie2_arr, Re2_arr ) = ( flat_samples[:,0], flat_samples[:,1], flat_samples[:,2], flat_samples[:,3], flat_samples[:,4])
	Ie1_mc, Re1_mc, ne1_mc = np.median(Ie1_arr), np.median(Re1_arr), np.median( ne1_arr )
	Ie2_mc, Re2_mc = np.median(Ie2_arr), np.median( Re2_arr)

	mc_fits = [ Ie1_mc, Re1_mc, ne1_mc, Ie2_mc, Re2_mc ]

	fig = corner.corner(flat_samples, bins = [100] * n_dim, labels = param_labels, quantiles = [0.16, 0.84], 
		truths = [ Ie1, Re1, Ne1, Ie2, Re2 ], levels = (1 - np.exp(-0.5), 1-np.exp(-2), 1-np.exp(-4.5) ), 
	...
	plt.savefig('/home/xkchen/low_BCG_star-Mass_%s-ban_2-sersic_out-2.1_fit_params.jpg' % band[tt], dpi = 300)
	plt.close()


	fit_line = com_sersic( com_r, Ie1_mc, Re1_mc, ne1_mc, Ie2_mc, Re2_mc, Ne2)
	fit_1 = sersic_func( com_r, Ie1_mc, Re1_mc, ne1_mc )
	fit_2 = sersic_func( com_r, Ie2_mc, Re2_mc, Ne2)
	...
	values = [ Ie1_mc, Re1_mc, ne1_mc, Ie2_mc, Re2_mc, Ne2]
	fill = dict(zip( keys, values) )
	out_data = pds.DataFrame( fill, index = ['k', 'v'])
	out_data.to_csv( out_path + 'low_BCG_star-Mass_%s-band_lim-n=2.1_mcmc_fit.csv' % band[tt])
	...
	plt.savefig('/home/xkchen/low-M-star_%s-band_M-star_sample_lim-out-n=2.1_BG-sub-SB_fit.jpg' % band[ tt ], dpi = 300)
	plt.close()
...
for tt in range( 3 ):
	...
	## read pre-fit params
	m_dat = pds.read_csv( out_path + 'low_BCG_star-Mass_%s-band_lim-n=2.1_mcmc_fit.csv' % band[ tt ],)
	...
	## array saving
	file_name = out_path + 'high_BCG_star-Mass_%s-band_2-sersic_out=2.1_fit_mcmc_arr.h5' % band[tt]
	...
	plt.savefig('/home/xkchen/high_BCG_star-Mass_%s-ban_2-sersic_out-2.1_fit_params.jpg' % band[tt], dpi = 300)
	plt.close()
	...
	fill = dict(zip( keys, values) )
	out_data = pds.DataFrame( fill, index = ['k', 'v'])
	out_data.to_csv( out_path + 'high_BCG_star-Mass_%s-band_lim-n=2.1_mcmc_fit.csv' % band[ tt ])
	...
	plt.subplots_adjust( left = 0.15,)
	plt.savefig('/home/xkchen/high-M-star_%s-band_M-star_sample_lim-out-n=2.1_BG-sub-SB_fit.jpg' % band[ tt ], dpi = 300)
	plt.close()
