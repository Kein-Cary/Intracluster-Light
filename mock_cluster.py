...
data.to_csv('target_BCG_catalogue.csv')
'''
# divide by z == 0.25
...
def SB_pro():
	...
	bx.errorbar(pR_mooth, SB_mooth, yerr = [err0_mooth, err1_mooth], xerr = None, ls = '', fmt = 'g.', label = ' smooth image ', alpha = 0.5)
...
def mock_ccd():
	...
	for dd in range(Nz):
		...
		fil = fits.Header(ff)
		## save the mock image [flux is in unit of ADU]
