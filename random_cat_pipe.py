...
def mask_A(band_id, z_set, ra_set, dec_set):
	...
	for q in range(Nz):
		...
		try:
			...
			hdu.data = mirro_A
			hdu.header = head_inf
			hdu.writeto(load + 'random_cat/mask_img/A_mask_data_%s_ra%.3f_dec%.3f_z%.3f.fits'%(band[kk], ra_g, dec_g, z_g),overwrite = True)
			...
			plt.savefig(home + 'fig_class/random_pont/A_mask_%s_ra%.3f_dec%.3f_z%.3f.png'%(band[kk], ra_g, dec_g, z_g), dpi = 300)
			plt.close()
